...
public class Configuration implements SourceResolver, NotationSet {
    ...
    private final ParseOptions defaultParseOptions = new ParseOptions();
    ...
    private final Map<String, FunctionAnnotationHandler> functionAnnotationHandlers = new HashMap<>();
...
//    }
    ...
    /**
     * Create an instance of a ResourceResolver with a specified class name.
     * Note that this method does not register the URIResolver with this Configuration.
     *
     * @param className The fully-qualified name of the ResourceResolver class
     * @return The newly created ResourceResolver
    ...
    public IntPredicateProxy getValidCharacterChecker() {
        if (xmlVersion == XML10) {
            return IntPredicateLambda.of(ch -> XMLCharacterData.isValid10(ch));
        } else {
            return IntPredicateLambda.of(ch -> XMLCharacterData.isValid11(ch));
        }
    }
    ...
    public BuiltInFunctionSet getXPath40FunctionSet() {
     ...
     * @since 8.4. From 11.1 this method has no effect.
     */
...
//        }
    ...
    public void readMultipleSchemas(PipelineConfiguration pipe, String baseURI, List<String> schemaLocations, String expected)
            throws SchemaException {
        needEnterpriseEdition();
    }
    ...
    public XPathParser newExpressionParser(String language, boolean updating, int languageVersion) throws XPathException {
        if ("XQ".equals(language)) {
            ...
            } else if (languageVersion == 40 || languageVersion == 31 || languageVersion == 30 || languageVersion == 10) {
                XQueryParser parser = new XQueryParser(31);
                parser.setLanguage(XPathParser.ParsedLanguage.XQUERY, Math.max(languageVersion, 31));
                return parser;
            } else {
                throw new XPathException("Unknown XQuery version " + languageVersion);
            }
        } else if ("XP".equals(language)) {
            if (languageVersion == 40 || languageVersion == 31 || languageVersion == 30 || languageVersion == 305 || languageVersion == 20) {
    ...
    public java.util.function.Function<SequenceIterator, FocusTrackingIterator> getFocusTrackerFactory(
        ...
        //Written this way for Java-C# conversion
    ...
    public synchronized JavaExternalObjectType getJavaExternalObjectType(Class<?> theClass) {
        return new JavaExternalObjectType(theClass);
    }
        ...
        if (booleanFeatures.contains(code)) {
            ...
            switch (code) {
                case FeatureCode.ALLOWED_PROTOCOLS:
                    ...
                    ResourceResolver rsrcResolver = getResourceResolver();
                    if (rsrcResolver instanceof CatalogResourceResolver) {
                        ((CatalogResourceResolver) rsrcResolver).setFeature(ResolverFeature.ACCESS_EXTERNAL_DOCUMENT, (String) value);
                    }
                    break;
                ...
                case FeatureCode.OPTIMIZATION_LEVEL:
                    ...
                    internalSetBooleanProperty(FeatureCode.GENERATE_BYTE_CODE, FeatureKeys.GENERATE_BYTE_CODE,
