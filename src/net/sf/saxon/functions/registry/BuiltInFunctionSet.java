...
//        }
                    ...
                    if (localName.equals("concat")) {
                        ...
                        for (int i=0; i<staticArgs.length; i++) {
                            if (staticArgs[i] instanceof StringLiteral) {
                                ...
                                RoleDiagnostic role = new RoleDiagnostic(RoleDiagnostic.FUNCTION, "concat", i);
                                a2[i] = CardinalityChecker.makeCardinalityChecker(staticArgs[i], StaticProperty.ALLOWS_ZERO_OR_ONE, role);
    ...
    public SystemFunction makeFunction(String name, int arity) throws XPathException {
        ...
        Class<? extends SystemFunction> functionClass = entry.implementationClass;
        SystemFunction f = instantiate(name, functionClass);
        f.setDetails(entry);
        f.setArity(arity);
        return f;
    }

    private SystemFunction instantiate(String name, Class<? extends SystemFunction> functionClass) {
        SystemFunction f;
        try {
